        ...
        if self.length == 1:
            ...
            if dimension_W == True:
                for i in range(
                    ...
                    offset[:, i * 3 + 2, :, :, :] = temp[:, i, :, :, :]  # W
        # 调试
        # print("offset_shape__{}".format(offset.shape))
...
# 改的代码
class DeformConvPack_d_new(DeformConv_d):

    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding, dimension='THW',
                 dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True, lr_mult=0.1):
        super(DeformConvPack_d_new, self).__init__(in_channels, out_channels,
                                             kernel_size, stride, padding, dimension, dilation, groups, deformable_groups,
                                             im2col_step, bias)
        self.dimension = dimension
        self.length = len(dimension)
    ...
    def forward(self, input):
        ...
        # print("原始offset_shape={}".format(temp.shape))
        ...
        if self.length == 3:
            ...
            # for i in range(
            #         self.deformable_groups * self.kernel_size[0] * self.kernel_size[1] * self.kernel_size[2]):
            #     offset[:, i * 3, :, :, :] = temp[:, i * 3, :, :, :]  # T
            #     offset[:, i * 3 + 1, :, :, :] = temp[:, i * 3 + 1, :, :, :]
            #     offset[:, i * 3 + 2, :, :, :] = temp[:, i * 3 + 2, :, :, :]
        ...
        # 调试
        # print("修改后offset_shape__{}".format(offset.shape))
